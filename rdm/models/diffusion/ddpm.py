...
class RDM(DDPM):
    ...
    def instantiate_pretrained_enc(self, config):
        self.pretrained_encoder = models_pretrained_enc.__dict__[
            config.params.pretrained_enc_arch
        ...
        if "moco" in config.params.pretrained_enc_arch:
            self.pretrained_encoder = models_pretrained_enc.load_pretrained_moco(
                self.pretrained_encoder, config.params.pretrained_enc_path
            )
            ...
            self.pretrained_encoder = models_pretrained_enc.load_pretrained_mae(
                self.pretrained_encoder, config.params.pretrained_enc_path
            )
        ...
        for zd in tqdm(samples, desc=desc):
            denoise_row.append(
                self.decode_first_stage(
                    zd.cuda(), force_not_quantize=force_no_decoder_quantization
                )
            )
    ...
    def get_input(
        ...
        with torch.no_grad():
            ...
            x_normalized = torch.nn.functional.interpolate(
                x_normalized, 224, mode="bicubic", align_corners=False
            )
